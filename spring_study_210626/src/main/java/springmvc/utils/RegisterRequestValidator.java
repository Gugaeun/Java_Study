...
public class RegisterRequestValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		RegisterRequest reqReg = (RegisterRequest)target;
		
		if(reqReg.getEmail() == null || reqReg.getEmail().trim().isEmpty()) {
			errors.rejectValue("email", "required");
		} else {
			// 정규 표현식을 이용해서 이메일이 올바른지 확인
			Matcher matcher = pattern.matcher(reqReg.getEmail());
			if(!matcher.matches()) {
				// 정규 표현식이 일치하지 않을 경우
				errors.rejectValue("email", "잘못된 이메일 형식입니다");
			}
		}
	}
